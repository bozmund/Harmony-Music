class YoutubeApiClient {
  ...
  // FORK PATCH (visionos): the only client that still returns downloadable
  // media URLs. In August 2026 YouTube began gating the others behind a
  // proof-of-origin token: extraction succeeds, then the URL serves ~1MB from
  // offset 0 and answers any open-ended read — which is what a player issues —
  // with 403. yt-dlp added this client for the same reason and marks it as
  // requiring neither a JS player nor a PO token, which is what makes it
  // usable here: no signature deciphering means no JavaScript engine, so
  // extraction stays on a background isolate instead of blocking the UI
  // thread. Values mirror yt-dlp's `visionos` INNERTUBE_CLIENTS entry; if
  // playback starts 403ing again, re-check them against upstream first.
